...
%                     end
        ...
        function obj = tuneGradient(obj)
            ...
            wl.Va = sqrt(obj.loss.Va) * obj.vmvaWeight * 2 / wl.total; % * 2 * obj.numBus; the number five is the average degree of a distribution network times two  * 5
...
%             end
        ...
        function obj = buildHessianVa(obj, snap, bus)
            ...
            lossThis = (obj.dataO.Va(bus, snap) - obj.data.Va_noised(bus, snap));
            obj.loss.Va = obj.loss.Va + lossThis^2 * obj.sigma.Va(bus).^(-2);
            gradVaThis = obj.sigma.Vm(bus).^(-2) * lossThis * g;
            obj.gradVa = obj.gradVa + gradVaThis;
            HVaThis = obj.sigma.Vm(bus).^(-2) * (g * g');
