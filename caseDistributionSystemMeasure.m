...
%             h_Va(bus) = h_Va(bus)-sum(GBThetaQ(bus, :) * obj.dataO.Vm(:, snap) * obj.dataO.Vm(bus, snap));
        ...
        function obj = tuneGradient(obj)
            ...
            if ~obj.tuneGrad
                ...
                wb.Vm = mean(abs(obj.boundA.total(1+obj.numFIM.G+obj.numFIM.B-2*obj.numFIM.del:obj.numFIM.G+obj.numFIM.B+obj.numFIM.Vm-2*obj.numFIM.del)));
                wb.Va = mean(abs(obj.boundA.total(1+obj.numFIM.G+obj.numFIM.B+obj.numFIM.Vm-2*obj.numFIM.del:end)));
            else
...
%             delta = delta1 * obj.lambda/(1+obj.lambda) + delta2 * 1/(1+obj.lambda);
            ...
            par(id) = obj.parChain(id, obj.iter) - delta; % 这一步产生了冲突，维度不一样啊
            G = par(1:obj.numGrad.G);
            B = par(1+obj.numGrad.G:obj.numGrad.G+obj.numGrad.B);
%             G(G>0) = 0;
%             B(B<0) = 0;
