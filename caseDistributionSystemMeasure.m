classdef caseDistributionSystemMeasure < caseDistributionSystem
    ...
    properties
        ...
        maxLambda           % the maximum value
        minLambda           % the minimum value
...
%                     elseif (mean(obj.lossChain(1, obj.iter-9:obj.iter-5)) / mean(obj.lossChain(1, obj.iter-4:obj.iter)) > 1.001)
%                         obj.step = min(obj.step * 1.2, obj.stepMax);
%                         obj.momentRatio = max(obj.momentRatio - 0.1, obj.momentRatioMin);
...
%                 end
        ...
        function obj = identifyOptLMPower(obj)
            ...
            % equations
            obj.lambda = obj.numBus * obj.numSnap * 1e6;
            obj.maxIter = 2000;
            obj.thsTopo = 0.01;
            obj.Topo = true(obj.numBus, obj.numBus);
            obj.Tvec = logical(obj.matOfColDE(obj.Topo));
...
%             obj.dataO.Va = obj.data.Va;
        ...
        function obj = updateParLMPower(obj)
            ...
            H1 = obj.H;% + diag(obj.lambda * abs((obj.grad).^(-1)));
...
%             end
            ...
            try
                if obj.lossChain(1, obj.iter) < obj.lossChain(1, obj.iter-1)
                    obj.lambda = max(obj.lambda / 2, obj.minLambda);
                else
                    obj.lambda = min(obj.lambda * 3, obj.maxLambda);
                end
            catch
            end
        end
